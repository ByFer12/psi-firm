...
import { 
  ...
  Download, ArrowLeft, Brain, Stethoscope,
...
// Definir tipo para la lista de pacientes
interface Patient {
  id: number;
  firstName: string;
  lastName: string;
...
// Definir tipos para los componentes auxiliares
...
export const Patients = ({ patientsList }: PatientsProps) => {
  ...
  const getModalityLabel = (id: number): string => {
    const map: Record<number, string> = { 1: 'Individual', 2: 'Pareja', 3: 'Familiar', 4: 'Grupal' };
  ...
  const loadFullHistory = async (recordId: number) => {
    try {
      ...
      console.log("Expediente seleccionado: ", res.data);
      setRecord(res.data);
      setView('detail');
                        ...
                        {ses.assignedTasks && ses.assignedTasks.length > 0 && (
                          <div className="mt-3">
                            ...
                            <ul className="list-disc list-inside space-y-1 text-sm text-slate-700">
                              {ses.assignedTasks.map((task: Task) => (
                                <li key={task.id}>
                ...
                <div className="space-y-3">
                  {record.prescriptions && record.prescriptions.length > 0 ? record.prescriptions.map((pre, idx) => (
                    <div key={idx} className="bg-purple-50 p-3 rounded border border-purple-100">
                      ...
                      <span className="text-xs text-slate-500 block"><strong>Nombre genérico:</strong> {pre.medication.genericName} </span>
                      ...
                      <p className="text-xs text-slate-500"> <strong>Presentación:</strong> {pre.medication.presentation}</p>                                     
    ...
    <div className="space-y-6">
      ...
      {loading ? <div className="text-center py-20 text-slate-400">Cargando...</div> : (
        <div className="grid md:grid-cols-3 gap-6 align-start"> 
          {patientsList
            .filter((p: Patient) => p.firstName.toLowerCase().includes(searchTerm.toLowerCase()))
            .map((patient: Patient) => (
                  ...
                  {loadingList ? (
                    ...
                    <div className="space-y-2">
                      ...
                      {patientRecordsList.map((rec) => (
                        <button 
                          key={rec.id} 
                          onClick={() => loadFullHistory(rec.id)}
...
const CardBlock = ({ title, icon, children }: CardBlockProps) => (
...
const InfoCard = ({ label, value, highlight }: InfoCardProps) => (
...
const ScaleRow = ({ label, val, labels }: ScaleRowProps) => (
