...
import { 
  ...
  CalendarDays, Pill, Trash2, Search, CheckCircle2, XCircle,
  FileText
} from 'lucide-react';
...
export const Sesion = ({ onNavigateToOpening, initialAppointment }: { onNavigateToOpening?: (patient: any) => void, initialAppointment?: any }) => {
  ...
  useEffect(() => {
    if (formData.nextSessionDate && currentAppointment) {
      // CORREGIDO: Ahora coincide con la firma actualizada
      fetchBusyHours(formData.nextSessionDate, currentAppointment.psychologistId || 1);
    }
  ...
  useEffect(() => {
    if (reprogramForm.date && reprogramAppt) {
      // CORREGIDO: Ahora coincide con la firma actualizada
      fetchBusyHours(reprogramForm.date, reprogramAppt.psychologistId || 1, true);
    }
  }, [reprogramForm.date]);
  ...
  // CORREGIDO: Se agregó psychologistId como segundo argumento
  const fetchBusyHours = async (date: string, psychologistId: number, isReprogram: boolean = false) => {
    try {
      ...
      const occupied = res.data
        .filter((a: any) => {
          ...
          // Opcional: Filtrar también por el ID del psicólogo si la API devuelve citas de todos
          if (a.psychologistId && a.psychologistId !== psychologistId) return false;
      ...
      if (isReprogram) {
        setReprogramBusyHours(occupied);
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("DATOS A ENVIAR:", {
        citaID: currentAppointment.appointmentId,
...
const AppointmentCard = ({ appt, onStart, onReprogram, isFuture = false }: AppointmentCardProps) => (
